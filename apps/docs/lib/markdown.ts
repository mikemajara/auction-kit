...
const documentPath = async (slug: string): Promise<string> => {
  if (Settings.gitload) {
    // For GitHub, try ${slug}.mdx first, then ${slug}/index.mdx
    const directPath = `${GitHubLink.href}/raw/main/contents/${slug}.mdx`
    try {
      ...
      // Fall through to index.mdx
    }
    return `${GitHubLink.href}/raw/main/contents/${slug}/index.mdx`
  } else {
    // For local files, try ${slug}.mdx first, then ${slug}/index.mdx
    const directPath = path.join(process.cwd(), "/contents/", `${slug}.mdx`)
    try {
